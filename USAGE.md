...
| `MIRROR_QUEUE_URL` | **Yes** | Queue identifier: an SQS queue URL or a Pub/Sub `projects/<p>/topics/<t>` resource. Still required by the env layer under `memory`, but ignored (it has no external queue). |
