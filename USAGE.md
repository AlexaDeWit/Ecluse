...
| `ECLUSE_OSV_SCRATCH_DIR` | No | `data/osv-scratch` | Local directory where Écluse Pilot compiles the `osv.db` SQLite file before uploading. |
| `ECLUSE_PILOT_S3_BUCKET` | Conditionally | Pilot only | Name of the S3 bucket where Écluse Pilot uploads the compiled `osv.db` file. Required if deploying Pilot. |
