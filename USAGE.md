...
5. **Fence egress, keep metadata reachable.** Default-deny outbound, allowing only your
   upstreams, the mirror target, the advisory bucket (the proxy needs `s3:GetObject` on
   `ECLUSE_VULNERABILITY_DATABASE_BUCKET` to sync `osv.db`, when configured), and the
   metadata endpoint; reach CodeArtifact and S3 over **VPC endpoints**; require **IMDSv2
   with hop limit 1**. Do **not** block the metadata endpoint
...
| `ECLUSE_CVE_DB_POLL_INTERVAL` | No | `60` | Proxy only: how often each configured ecosystem's sync task polls the bucket for a fresh advisory database (a cheap conditional `HEAD`). Deliberately independent of, and more frequent than, Pilot's `ECLUSE_CVE_SYNC_INTERVAL`: matching them would nearly double the worst-case advisory age. |
...
| `ECLUSE_OSV_DATA_DIR` | No | `data/osv` | Directory for the OSV advisory databases: where Pilot compiles them, and where the proxy lands its synced per-ecosystem artifacts. Note the proxy's active artifact can briefly exceed what `ls` shows during a swap: the superseded file is unlinked while its last readers finish, so `du` counts it until the drained connection closes and the kernel reclaims it. |
...
- **Health probes.** `GET /livez` reports process liveness (a stalled mirror worker fails
  ...
  additionally waits for **every configured ecosystem's first advisory sync**, a one-way
  flip per ecosystem, so it never flaps once ready; the listener serves throughout (an
  absent advisory database only ever abstains into deny-by-default), so the gate governs
  what a load balancer routes, never what the process can answer. Consequence: mounting an
  ecosystem whose artifact Pilot never publishes declares a sync that never arrives, and
  the pod honestly never reports ready. The npm liveness probe `GET /-/ping` is answered
