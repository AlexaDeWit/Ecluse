...
In addition to the long-running roles, `ecluse pilot compile --out DIR` runs a single OSV compilation and exits: it fetches one ecosystem's advisory export (`--ecosystem`, default `npm`; `--source URL` points it at a mirror or a fixture archive instead of the canonical osv.dev export), writes the `osv.db` artifact into `DIR`, and exits non-zero on failure, so it is safe to script and to schedule. With `--upload` it also publishes the artifact to the configured vulnerability-database bucket, making one invocation a complete sync cycle; requesting `--upload` without a configured bucket aborts immediately.
