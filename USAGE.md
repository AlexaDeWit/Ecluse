...
| `ECLUSE_OSV_URL` | No |  | The URL of the OSV ecosystem zip file (e.g., `https://osv-vulnerabilities.storage.googleapis.com/npm/all.zip`). |
| `ECLUSE_OSV_DB_PATH` | No | `osv.db` | The output path for the compiled SQLite database. |
| `ECLUSE_CVE_SYNC_INTERVAL` | No | `3600` | How often the Écluse Pilot singleton refreshes the OSV database from upstream. Minimum floor of 1 hour (3600s) is enforced. |
