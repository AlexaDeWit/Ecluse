...
for that complexity-scaling assertion. Building the filter plan runs the engine's
effectful rule sweep, so the filter+serve benches are 'IO'.
-}
...
benchmarks loaded =
    bgroup "serve (filter + url-rewrite + etag)" $
