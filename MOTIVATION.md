...
> **In short.** A modern build pulls in thousands of packages other people wrote, and any
> one malicious or hijacked version can reach an enormous number of builds before anyone
> notices. Here's the thing though: most bad publishes get caught and pulled fast, so the
> real danger lives in a narrow window between when something is published and when it's
> removed. So Écluse doesn't try to *detect* malice. It stays out of that window: hold
> public packages behind a short **freshness quarantine**, under a **deny-by-default**
> policy, enforced at a **single chokepoint** that CI and developers both have to pass
> through. When a malicious package gets disclosed, you compare its lifetime against your
> quarantine window instead of combing logs to learn whether you got hit. To do that
> *without* either blocking good packages or briefly serving bad ones, Écluse uses a
> **three-registry** design that **composes in front of the registry you already run**
> instead of replacing it, and **delegates storage** to that registry rather than hosting
> anything itself. It's **free and open**: offered, not sold.
>
> This is the *why*. The *how* lives in the [architecture docs](docs/architecture.md), and
...
behaviour. That's a hard, never-certain, perpetually moving target, and "we think it's
...
where the name comes from: *écluse* is French for a canal lock. Not a wall that blocks, but
a controlled passage every dependency is held in and cleared through before it's let
forward. The goal is **resilience**: shrinking the blast radius of a bad publish, not
detecting malware.
...
want to be honest about that. **The commercial repository-firewall and curation platforms
do exactly this**, off the shelf: an age-based quarantine, enforced at the proxy. If that's
what you need and you can fund it, buying is a legitimate, working answer. The catch isn't
...
- **Commercial repository-firewall and curation platforms** *do* sell exactly this
  ...
  one capability you adopt, operate, and pay for a second registry, most of which
...
- **Cryptographic provenance** is valuable and complementary, but it attests *where* a
  package was built, not *whether* it's safe.
...
Serve only what's already been replicated, and a legitimate package gets **404'd** until
the mirror catches up. Serve eagerly to avoid that, and an unvetted version gets **served
and culled only afterwards**, which is too late. Neither one is acceptable.
...
- On a **miss**, the public upstream is consulted, and the version is served *only if it
  ...
  qualifier is the whole trick: it's what makes "no false 404" and "no serving fresh
  malware" both true at the same time.
- Serving on a miss also **enqueues replication**: demand-driven, so only what's actually
  used ever gets copied (no prediction, no wholesale mirror), and the next request for it is
