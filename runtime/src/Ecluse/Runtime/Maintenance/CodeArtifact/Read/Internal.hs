...
{- | The observation evidence and the page projections behind
"Ecluse.Runtime.Maintenance.CodeArtifact.Read", which documents the read-only half and
re-exports the curated surface. Importing this module opts out of that stability promise, the
convention @text@ and @bytestring@ use, so production code imports the public one.
-}
module Ecluse.Runtime.Maintenance.CodeArtifact.Read.Internal (
