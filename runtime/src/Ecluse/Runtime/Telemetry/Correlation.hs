...
('Ecluse.Runtime.Log.formatDdTraceId' \/ 'Ecluse.Runtime.Log.formatDdSpanId'), and stays free of any
OpenTelemetry dependency. This module is the IO half that "Ecluse.Runtime.Log" deferred: it
...
("Ecluse.Runtime.Telemetry.Resolve") and carried as a span-less 'DdContext' (the
...
table the SDK configuration uses ("Ecluse.Runtime.Telemetry.Resolve"), so the log identity
