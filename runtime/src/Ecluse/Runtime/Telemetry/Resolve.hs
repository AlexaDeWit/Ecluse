...
{- | Telemetry configuration resolution -- the boot-time substrate that sits between the
operator's environment and the OpenTelemetry SDK.
...
What the SDK does when an export /fails/ is a separate concern, and lives in
"Ecluse.Runtime.Telemetry.ExportFailure". The configuration model is described in
