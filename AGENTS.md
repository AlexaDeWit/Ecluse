...
- **Releases attest provenance + SBOM.** Each published image carries keyless cosign attestations bound to the digest — a SLSA v1.0 provenance and an SBOM (`sbomnix` over `.#ecluse-bin`), via `make sbom` + `make attest`. Consumer verification recipe is in [`README.md`](README.md); details in [`CONTRIBUTING.md`](CONTRIBUTING.md) → "Supply-chain attestations".
