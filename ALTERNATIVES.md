...
the only good one. The idea at its core (holding fresh packages behind a delay and applying a
policy at a chokepoint) has been reached independently by a range of projects and vendors,
several of them more mature than Écluse is today.

So this page is a genuine, good-faith guide. Each entry says what a tool offers and when you
might reach for it. If one of these fits you better than Écluse, that's a good outcome: use
it. For _why_ Écluse exists and how it differs, see [`MOTIVATION.md`](MOTIVATION.md).
...
- **[Verdaccio](https://verdaccio.org/)**: a lightweight, widely-adopted self-hosted npm
  ...
  fallback, and since v6.4 it ships a bundled, optional package filter: a minimum-age gate
  (`minAgeDays`), version blocklists, per-package allow exceptions, and replacement of a
  blocked version with an older one. That filter is first-party now, not only a third-party
  plugin, which makes Verdaccio a genuine open-source npm repository-firewall configuration for
  an npm-only team. _Reach for it_ when you want to run your own npm registry, npm is the only
  ecosystem you care about, and a metadata age gate on the packument is enough.

  Where Écluse differs: it merges versions across multiple upstreams semantically rather than
  falling back in order; its rules engine adds OSV-driven CVE denial (`DenyIfCVE`) with an
  immediate `AllowIfRemediatesCve` fast lane, so a fix ships the moment it lands; it replaces
  the advisory index atomically; and it mirrors on demand into a registry you already run,
  whereas Verdaccio caches into its own storage. Verdaccio is also npm-only, and its filter is
  metadata-only: it gates the packument but does not intercept tarball requests.
...
- **JFrog Artifactory** with **[Curation](https://jfrog.com/curation/)**: a full artifact
  ...
  package") gating and malicious-package blocking. JFrog describes it informally as a firewall
  for open-source packages. _Reach for it_ when you want one vendor-supported platform across
...
- **[Harness](https://www.harness.io/) Dependency Firewall**: a commercial, centrally-managed
  control that blocks risky open-source dependencies at the proxy. _Reach for it_ when you want
  central enforcement inside the Harness platform.
...
- **pkgwarden**: a hosted, commercial curated registry that quarantines new versions for 14
  ...
  registry with a fixed quarantine and CVE filtering, without self-hosting.
- **Managed cloud registries**, **[AWS CodeArtifact](https://aws.amazon.com/codeartifact/)** and **[Google Artifact Registry](https://cloud.google.com/artifact-registry)**, give
  ...
  for it_ as the backing store you most likely already run; Écluse delegates storage to exactly
...
it or hosting packages itself, and applies a deny-by-default freshness policy consistently,
with the operational goal that a malicious-package disclosure can be answered by comparing
timelines rather than auditing logs. npm is the first supported ecosystem; the core is
registry-agnostic, and PyPI and RubyGems are on the roadmap. It's also, today, early and
unproven (see [`MOTIVATION.md`](MOTIVATION.md) → _What Écluse is not_).

If a different point on these trade-offs serves you better, one of the tools above is the right
choice, and that's exactly the outcome this page hopes to help with.
