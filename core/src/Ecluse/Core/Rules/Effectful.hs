{- | The effectful rule tier, layered on the pure one in "Ecluse.Core.Rules".
...
it is __fail-closed__ — the version is 'Ecluse.Core.Rules.Types.Undecidable', not
...
"Ecluse.Core.Breaker" state machine, kept per source as a 'TVar' so repeated failures of
...
'Ecluse.Core.Rules.Types.EvalContext', so its timing is deterministic under test, and a
...
import Ecluse.Core.Rules (evalRulesWithPrecedence)
...
(higher wins), mirroring 'Ecluse.Core.Rules.Types.PrecededRule' for the pure tier. The
...
the pure one. Returns the same 'Decision' the pure 'Ecluse.Core.Rules.evalRules' would,
