...
{- | The outage state machine behind "Ecluse.Core.Rules.Outage": what one evaluation saw, the state
it folds into, the reports that fold emits, and the record of admissions already logged.

Importing this module opts out of the public surface's stability promises. It exists so a spec can
drive the fold directly, without a clock or a shared cell.
-}
module Ecluse.Core.Rules.Outage.Internal (
