...
order — see 'Ecluse.Core.Rules.evalRules' and 'PrecededRule'.
-}
data Rule
...
wins). 'Ecluse.Core.Rules.evalRules' selects the highest-precedence non-abstaining
rule; at equal precedence a deny beats an allow, and any remaining tie is broken
by rule identity rather than list order (see 'Ecluse.Core.Rules.evalRules').
...
The first three arms are the pure tier's; the effectful tier ("Ecluse.Core.Rules.Effectful")
