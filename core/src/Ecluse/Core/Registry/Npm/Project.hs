...
runs, so a version reached this way is identical to the same version reached through
'parsePackageInfoFromValue'. Pure and total — a @Value@ that is not a decodable npm
packument is a 'ParseError', exactly as 'parsePackageInfoFromValue' reports it.

This is the entry point the trusted-tarball serve leg uses: it needs one version's
artifact, and the private metadata is uncached and re-fetched per request, so paying
to project every version's integrity\/semver\/details only to consult one is wasted
work on that hot path. The version-count bound still applies via the count
'VersionProjected' carries.
-}
