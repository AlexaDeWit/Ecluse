...
spec = do
    describe "verifyIntegrity" $ do
        ...
        it "verifies an SRI (sha384)-only artifact against its sha384 (the worker computes sha384)" $
            -- The whole point of modelling sha384: the worker must RECOMPUTE it, so a
            -- sha384-admitted artifact verifies rather than staying uncomputable.
        ...
        it "verifies a raw SHA384-tagged digest against its hex sha384 (the tag arm, not SRI)" $
            -- A digest carried under the raw 'SHA384' tag (hex), distinct from the same
            -- hash inside an SRI string. The worker computes hex SHA-384 and matches it.
        ...
        it "REJECTS a sha384 SRI that does not match the fetched bytes (tamper guard)" $
            -- The tamper direction for the sha384 compute path: a real, well-formed sha384
            -- that is the digest of OTHER bytes must fail closed, naming the algorithm.
        ...
        it "prefers and verifies a co-present sha384 over a matching sha1 (strongest wins, and is computable)" $
            -- sha384 outranks sha1, so the gate selects it. The worker computes sha384, so
            -- it verifies against that rather than failing closed or downgrading to sha1.
        ...
        it "verifies a blake2b-only digest (the worker now computes blake2b-512)" $
            -- blake2b ranks at the top tier and the worker computes it, so a blake2b-only
            -- artifact verifies rather than failing closed as uncomputable.
        ...
        it "REJECTS a blake2b digest that does not match the fetched bytes (tamper guard, the new arm)" $
            -- The tamper direction for the blake2b compute path: a real, well-formed
        ...
        it "prefers and verifies a co-present blake2b over a matching sha1 (strongest wins, now computable)" $
            -- blake2b outranks sha1, so the gate selects it. It is computable, so the
            -- worker verifies against it rather than failing closed or downgrading to sha1.
        ...
        it "fails closed on an md5-only digest (the worker will not verify a broken hash)" $
            -- MD5 is cryptographically broken, so the worker will not compute it. An
            -- md5-only artifact fails closed, never admitted on the strength of a forgeable
            -- hash. MD5 is also below the public floor, so this never arises for an admitted
            -- public job. The fail-closed stance is defence in depth.
        ...
        it "names the algorithm in a plain (computable) digest mismatch too" $
            ...
            -- match. The detail names the algorithm via the SRI 'describe' arm, so the gate
            -- reports a genuine tamper with its algorithm, not anonymously.
        ...
        it "REJECTS an SRI whose base64 body matches only after case-folding (base64 is case-sensitive)" $
            -- The hex arms fold case, because hex is case-insensitive. An SRI carries a
