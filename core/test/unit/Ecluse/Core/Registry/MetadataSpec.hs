...
{- | The fetch-then-project step every mount's read operations share. A mount supplies the
fetch action and the projection, so this pins the fold between them.
-}
spec :: Spec
spec = describe "fetchThenProject" $ do
    it "hands the fetched body to the projection" $
        runStep (Right (RegistryResponse "the-body")) Right `shouldReturn` Right "the-body"
    ...
    it "returns a projection refusal as the mount phrased it" $
        runStep (Right (RegistryResponse "junk")) (const (Left MetadataUndecodable))
    ...
    it "asks the fetch action for the requested package, once" $ do
        ...
        let fetch name = modifyIORef' asked (name :) $> Right (RegistryResponse "b")
