...
identitySpec = describe "identityDenyNames" $ do
    it "reads a bare name whole" $
        denied [DenyByIdentity "left-pad"] `shouldBe` [unscopedNpm "left-pad"]

    it "drops the version from a name@version deny" $
        denied [DenyByIdentity "left-pad@1.3.0"] `shouldBe` [unscopedNpm "left-pad"]

    it "splits a scoped name at the last @, keeping the namespace" $
        denied [DenyByIdentity "@babel/core@7.0.0"] `shouldBe` [scopedNpm "babel" "core"]

    it "reads a bare scoped name whole, because its only @ leads it" $
        denied [DenyByIdentity "@babel/core"] `shouldBe` [scopedNpm "babel" "core"]
...
intersectionSpec = describe "candidateSet" $ do
    ...
    it "carries every name the loaded generation covers" $ do
        ...
        map (inCandidates candidates) [unscopedNpm "left-pad", scopedNpm "babel" "core"] `shouldBe` [True, True]
    ...
    it "carries a name no advisory covers only when a deny pins it" $ do
        ...
        map (inCandidates candidates) [unscopedNpm "lodash", unscopedNpm "underscore"] `shouldBe` [True, False]
    ...
    it "intersects a differently spelled advisory name, because both sides are canonicalised" $ do
        ...
        inCandidates candidates (scopedNpm "babel" "core") `shouldBe` True
    ...
    it "reads no name from an advisory spelling the ecosystem's grammar refuses" $ do
        ...
        inCandidates candidates (unscopedNpm "left-pad") `shouldBe` False
    ...
    it "sweeps the identity half alone when no generation is loaded" $ do
        ...
        map (inCandidates candidates) [unscopedNpm "left-pad", unscopedNpm "lodash"] `shouldBe` [True, False]
