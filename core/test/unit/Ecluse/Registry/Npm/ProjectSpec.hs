...
__public tarball fallback__ ('Ecluse.Core.Server.Pipeline.gatePublicVersion') uses: it
projects only the requested version from an already-decoded packument @Value@, never the
others. This block is the proof that narrowing the public fallback that way leaves the
security-decisive gate __unchanged__: for the same @Value@, requested name, and version,
the targeted projector must yield the __same__ outcome the whole-packument projection
does — the same projected version count (so the version-count response bound fires
identically), the same name-mismatch verdict, and the __same 'PackageDetails'__ (or the
same absence) the full projection holds at that version.

Because the public-fallback gate downstream — the artifact selected by filename
('artifactFor', mirrored here as 'selectArtifact'), the rule decision, and the
integrity-floor decision over it — is a __pure function of that 'PackageDetails'__,
identical details entail an identical selected 'Artifact' and an identical gate outcome
for every filename, ruleset, and floor. The cases pin the representative shapes the task
enumerates (multi-version, the requested version present and absent, a below-floor
SHA-1-only artifact, a hashless artifact, a non-conventional trailing-slash @artUrl@, and
a filename that matches and one that does not), and a generative property confirms the
equivalence over arbitrary fuzzed bodies on all three arms (present, absent, mismatched
name).
-}
...
targetedVersionParitySpec = describe "parseVersionDetailsFromValue (targeted) parity with the full projection" $ do
    ...
    it "agrees on a below-floor (SHA-1-only) artifact's details" $ do
...
layer's @gatePublicVersion@ took before it was narrowed to one version).
-}
...
{- | Select the artifact a filename names from a version's details — the pure step the
public-fallback gate @gatePublicVersion@ runs downstream ('artifactFor'), replicated
here to show identical details entail an identical selected 'Artifact'.
-}
