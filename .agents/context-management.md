...
This guide keeps an agent's limited context on the evidence the current decision needs.
...
Layers 2-4 are replaceable. Do not copy layer 1 or a whole design document into a checkpoint.
...
- Prefer `rg` and targeted semantic navigation over recursive file dumps. Bound git logs, issue
  lists, test output, and generated artefacts to the portion you need.
- For a noisy command, write its log to disk and read the error summary. A successful full log has
...
[`.agents/compact-prompt.md`](compact-prompt.md) as the instruction: it is the literal prompt. The
checkpoint keeps only the facts you cannot cheaply reconstruct. It drops copied policy, design
prose, successful logs, and narration. On resume, verify volatile state live. Re-read a referenced
source only when the next decision needs it.
