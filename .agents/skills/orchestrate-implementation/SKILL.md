...
2. **Build**. Brief an implementer subagent in its own worktree. Carry the architect's acceptance
   ...
   implementer opens the draft PR at its first push, reports the head SHA at once, and exits. It
   does not idle in a watch loop.
...
4. **Gate**. Own the watch. At the PR-open report, start one detached background watch on the PR
   (`gh pr checks <pr> --watch` in a background shell). It returns once, at the terminal state,
   and is the authoritative signal; an implementer's own report is secondary
   ([CI-verified batches](../../orchestration-strategy.md#ci-verified-batches-the-wide-parallel-mode)).
   ...
   gap (a suspend, a restart), sweep `gh pr checks` on every open PR: a gap kills background
   watches.
