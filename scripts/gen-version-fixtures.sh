...
# (core/test/unit/Ecluse/Core/VersionSpec.hs). This script needs the reference
...
# Curated version lists that exercise the fiddly corners of each grammar. They
# cover numeric ordering (10 vs 9), prerelease against release, and the
# per-ecosystem prerelease tie-break rules. They also cover build and local
# metadata, normalisation equivalences, and real-world versions from popular
# packages. Every entry MUST be accepted by its reference tool (node-semver /
# packaging / Gem::Version).
# Invalid and rejection cases are out of scope here: they belong to the
# must-not-parse work, and they would abort generation. The pairwise expansion
# is O(n^2), so keep each list to a few dozen carefully-chosen strings.
npm_versions=(
...
rubygems_versions=(
  ...
  # Gem::Version rewrites a hyphen to ".pre.".
  1.0.0-1 1.2.3-4
