...
[`npm.md`](npm.md). It builds a JSON type model of the wire format (see
[Type model](#11-type-model)). The model covers both directions: what Écluse
fetches as a Python client, and what it serves for `pip` to install from.

> **Terminology.** `pip` is the client. The registry is **PyPI** (the Python
> Package Index) at `https://pypi.org`. Two protocols matter. `pip` uses the
> **Simple / Index API** (PEP 503 HTML, PEP 691 JSON, PEP 700 extensions), the
> analogue of npm's abbreviated packument. The **JSON API**
> (`/pypi/{p}/json`) is the analogue of the full packument. Artifacts live on a
...
a normalised name over a flat list of **distribution files**. A version is the set
of files that share a release number. Two consequences dominate:
...
- A version has many files, not one tarball. Each release offers at most one
  **sdist** (`*.tar.gz` source) and zero or more **wheels** (`*.whl`, pre-built,
  ...
  list here.
- Resolution is the client's job, as in npm. The index returns every file. `pip`
  parses PEP 440 versions, filters by compatibility, wheel tags, and yank status,
  then picks. PyPI resolves no specifiers server-side (§8).
...
| The artifact | one `.tgz` per version | N `.whl` plus one `.tar.gz` per version |
| Integrity | `dist.integrity` (SRI), `shasum` | `hashes.sha256` (+ `md5`, `blake2b`) |
| "Current" pointer | `dist-tags.latest` (server-set) | none on the wire, the client computes it. `info.version` (JSON API) is the latest |
...
Three request shapes cover almost all install traffic:
...
- Artifacts and PEP 658 metadata: `https://files.pythonhosted.org`. A proxy must
  front (or rewrite to) both hosts. A client that resolves metadata through Écluse
  but pulls the bytes straight from `files.pythonhosted.org` bypasses the gate.
- Always HTTPS, HTTP/2.

### Project-name normalisation (PEP 503)

PEP 503 matches names case-insensitively and treats `_`, `-`, and `.` as equal.
...
A server must normalise the name and redirect. A client must normalise before it
requests, and follow the redirect.
...
application/vnd.pypi.simple.v1+html;q=0.1, text/html;q=0.01`, so it prefers JSON.
The JSON API (`/pypi/…/json`) is a separate endpoint, always `application/json`,
and never content-negotiated.
...
| `X-PyPI-Last-Serial` | `37059094` | monotonic serial of the last event on this project, also `meta._last-serial` / `last_serial` in bodies |
...
PyPI honours `Accept-Encoding: gzip`. It asks clients to set a descriptive
`User-Agent` and to prefer a mirror for high volume.

### Errors

An unknown project or version returns a plain `404` with no envelope, unlike
npm's `{error}`. On a Simple-API surface, deny by omitting the file or version
(§8), or with a `403`.
...
The wire carries no login, whoami, or token-lifecycle API. A publisher mints a
token in the web UI or through Trusted Publishing (CI OIDC, §9). This diverges
from npm.
...
packument: one document that covers the project and all its releases.
...
> - **`releases`**, "projects should shift to using the Index API". PyPI may
>   remove it. Prefer the Simple API for the version/file list.
> - **`downloads`**, always `-1`.
...
| `requires_dist` | string[] (PEP 508) | Dependencies with markers and extras, e.g. `"PySocks!=1.5.7,>=1.5.6; extra == \"socks\""`. |
...
`GET /simple/{project}/`. The endpoint `pip` uses, and the proxy's primary
surface. It has two equivalent representations.
...
The sdist has no `data-core-metadata`, because only wheels carry a METADATA file.
...
| `files[].hashes` | object`<alg,hex>` | 691 | Keyed by algorithm. `sha256` is always present. |
| `files[].requires-python` | string\|null | 691 | PEP 440 gate. |
| `files[].core-metadata` | bool \| `{sha256}` | 658/714 | If truthy, a `.metadata` file exists (see §6). `data-dist-info-metadata` is the old name for the same thing. PyPI emits both for compatibility (PEP 714). |
...
Prefer JSON, the richer and more modern surface. Fall back to HTML only for
ancient mirrors.
...
`requests 2.32.3` → 2 files (`bdist_wheel`, `sdist`). An unknown version → `404`.
...
- Wheels run no code on install. The installer unpacks them and executes
  nothing.
- An sdist runs a build backend (`setup.py` / PEP 517) at build time, which is
  arbitrary code. So the signal is "prefer wheels, treat sdist-only releases as
  higher risk". The file list carries it in `packagetype`, with no download.

---

## 7. The file (distribution) object

The file is the security-critical unit, the analogue of npm's `dist`. A version
has many, and the shape differs slightly between the JSON and Simple APIs.
...
| `python_version` | string | Wheel: a tag like `py3` or `cp310`. sdist: `source`. |
| `requires_python` | string\|null | PEP 440 gate. |
| `size` | number | Bytes. |
| `upload_time_iso_8601` | ISO-8601 | Publish timestamp. |
| `digests` | object | `{ sha256, md5, blake2b_256 }`. Verify `sha256`. |
| `yanked` / `yanked_reason` | bool / string\|null | PEP 592. |
| `has_sig` | bool | Legacy GPG flag, now always `false` (deprecated). |
| `comment_text` | string | Uploader note (usually empty). |

### Simple API file

The same artifact with leaner fields: `filename`, `url`, `hashes`,
`requires-python`, `core-metadata`, `provenance`, `size`, `upload-time`,
`yanked` (§5). The installer resolves against the Simple form.

### Filename encodes compatibility (PEP 425/427)

A wheel filename has this structure:
...
`pip` selects files whose tags match the running interpreter and platform. A pure
...
As with npm, the registry resolves no specifiers. `pip` resolves them
client-side.
...
The wire carries no `latest` pointer, unlike npm's `dist-tags.latest`.
...
3. Filter each candidate:
   ...
   - wheel tags (PEP 425) must match the platform, else `pip` falls back to the
     sdist and builds it.
   - yanked files (PEP 592): `pip` drops them unless the requirement pins that
     exact version (`==`). Yanked ≠ deleted, just invisible to ranges.
   - `pip` excludes pre-releases by default unless `--pre` or a pin.
4. Pick the highest remaining version and the best wheel (or sdist).
5. Resolve transitive dependencies from each candidate's `.metadata` (§6), else
   by downloading.
6. Download, verify `sha256`, install.

So availability means a usable, non-yanked file is present in the Simple index,
after compatibility filtering.

### Consequences for a proxy (both directions)

- **As a client**, `GET /simple/{p}/` (JSON) and read `versions`/`files`. Resolve
  PEP 440 yourself, and read dependencies from `.metadata` rather than wheels.
...
- **Policy shapes availability.** To hide a denied release, omit its files, and
  `pip` never considers it. To soft-block, mark files `yanked`, which keeps `==`
  pins and drops them from ranges. To hard-block, `403` the artifact.
  Deny-by-default is a filtered projection of the upstream index.
- **`upload-time` is the age signal**, in the Simple JSON (PEP 700) and the JSON
  API (`upload_time_iso_8601`). It is per file, not per version: a publisher can
  upload one version's wheels at different times.

---

## 9. Authentication (in theory)

No token available. All read endpoints are anonymous: every probe above succeeded
with no credentials. Auth gates writes only.
...
index URL. `pip` takes credentials from, in order: the URL
(`https://user:pass@host/simple/`), `keyring`, then `~/.netrc`. It sends
`Authorization: Basic <base64(user:pass)>`. Reads use no bearer token, OTP, or
login handshake.

### Writing (upload)

Upload endpoint: `POST https://upload.pypi.org/legacy/` (the "legacy" name is
historical, the endpoint is current). `twine` is the reference client. Two
mechanisms:
...
1. **API tokens**, HTTP Basic with a fixed username and the token as password:
   ...
   ```
   A publisher creates them in the web UI (account- or project-scoped). PyPI
   removed username/password upload, so `__token__` or Trusted Publishing is
   mandatory.
...
- A read proxy to public PyPI needs no credentials, simpler than npm.
- **Private upstream**: attach `Authorization: Basic …` (static token or
  CodeArtifact's IAM-derived credential, same Basic shape).
- **Mirror or upload** (if the proxy publishes): `POST .../legacy/` with
  `__token__` Basic, or mint a token through OIDC.
- The proxy serves installs without any of PyPI's write or auth endpoints.
...
The write path sits off the proxy's critical path, because Écluse delegates
storage.
...
- **Yank and unyank**: a publisher sets the PEP 592 flag through the web UI or
  the API. No public wire endpoint compares to npm dist-tags.
- **Provenance**: a publisher submits attestations (PEP 740) alongside the upload
  under Trusted Publishing. PyPI serves them at
  `/integrity/{p}/{v}/{file}/provenance`.

---

## 11. Type model

A JSON type model for the wire format. It shares vocabulary with [`npm.md`
§11](npm.md#11-type-model) for easy comparison. Be lenient on input: ignore
unknown keys, tolerate the HTML and JSON dual forms, and accept the
`core-metadata` / `data-dist-info-metadata` alias. Be strict on output. ⚠️ marks
a shape that differs materially from the npm wire model.
...
> **Cross-ecosystem note.** Four shapes differ most from npm. A version owns N
> artifacts here, not one (`Dist` → `[File]`). Names are PEP 503 normalised
> rather than scoped. The publish timestamp is per file here, where npm has the
> packument `time` map. The install-time-execution signal is
> `packagetype == sdist` rather than npm's `hasInstallScript`.
...
- [ ] Serve coherent `files`/`versions` so the client runs PEP 440 and tag
      selection locally: correct `sha256`, `requires-python`, `yanked`, `size`,
      and `upload-time` per file (§8).
- [ ] Serve (or rewrite to) artifacts on the files host, plus the `.metadata`
      companions, with immutable cache headers.
- [ ] Optionally serve the JSON API (`/pypi/{p}/json`, `/pypi/{p}/{v}/json`) and
      `vulnerabilities`.
- [ ] Return a plain `404` for an unknown project or version. Deny by omitting
      files, or with a `403` on the artifact.
- [ ] Send `ETag` and `X-PyPI-Last-Serial` so clients and mirrors revalidate
      cheaply.

### To be a correct Python **client** (fetch from upstreams)

- [ ] Normalise names (PEP 503). Request Simple JSON with
      `Accept-Encoding: gzip`. Set a descriptive `User-Agent`.
- [ ] Resolve PEP 440 specifiers, wheel tags (PEP 425), and `requires-python`
      locally, and exclude yanked files (PEP 592). Never expect an upstream to
      do it.
...
- [ ] Project upstream indexes into a filtered index that reflects policy
      decisions (omit or yank denied files, preserve `upload-time`).
