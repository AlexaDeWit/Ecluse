...
and the npm CLI, and skips every case as `pending` when `ECL_E2E_IMAGE` is unset.
...
id. That id comes from `ECL_TEST_SCOPE`, which every container-running target sets:
`task test-integration`, `task test-e2e`, and the `coverage` tier `task check` runs.

Every harness and CI variable uses the `ECL_` prefix, never `ECLUSE_`. The config loader claims the
whole `ECLUSE_` prefix and aborts the boot on any variable under it that is not a config key, so a
harness variable on that prefix would stop the very proxy the tests are booting.
