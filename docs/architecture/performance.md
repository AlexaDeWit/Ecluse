...
| `cached-public-hit` | the same `GET` over the corpus mix, with the anonymous public origin served from the **warm metadata cache** (bound holds the whole set) | the cheap, common high-throughput path: the public fetch and decode are elided, and a recurring (public entry, private content, plan) triple serves the memoised assembled bytes with no re-assembly or re-encode |
| `revalidate-not-modified` | the heaviest corpus packument, every request echoing a primed `ETag` as `If-None-Match`, answered `304` off the derived validator | the conditional short-circuit: the per-request private fetch and plan without assembly, encode, or output hashing (the dominant metadata shape for CI fleets that restore npm's local cache) |
