...
| `PUBLICATION_TARGET_TOKEN` | No | Static credential for the publication target when it is not reached with the client's forwarded token. The default publish credential model is **passthrough** — the publisher's own token. **Requires `PROXY_AUTH_TOKEN`:** the static token makes Écluse publish under its **own** credential for a caller who forwards none, so a `PUBLICATION_TARGET_TOKEN` set without an edge token is **refused at boot** (an open edge would otherwise let an unauthenticated client publish within scope). See [Access model](access-model.md#publishing-the-publication-target-passthrough-write). |
...
`PROXY_MIN_TRUSTED_INTEGRITY` sets the floor (default `sha256`). Unlike the public floor it
