...
| `ECLUSE_MOUNTS__NPM__MIRROR_CODE_ARTIFACT_REGION` | `codeartifact` provider only | The region of the CodeArtifact domain. Resolution order: this key → the mirror-target host (its authoritative region) → `ECLUSE_AWS_REGION`. |
...
| `ECLUSE_AWS_ENDPOINT_URL_SQS` | No | SQS endpoint override (the AWS-SDK-standard variable). Set to target a local emulator (`ministack`) or a VPC endpoint; the released image uses the same key with no test-only code path. Takes precedence over `ECLUSE_AWS_ENDPOINT_URL`. With an override set, requests are signed with `ECLUSE_AWS_ACCESS_KEY_ID`/`ECLUSE_AWS_SECRET_ACCESS_KEY` (an emulator is off the ambient role chain). |
| `ECLUSE_AWS_ENDPOINT_URL` | No | Generic AWS endpoint override (the AWS-SDK-standard variable), used for SQS when `ECLUSE_AWS_ENDPOINT_URL_SQS` is unset. |
...
credentials above: `ECLUSE_AWS_REGION` / instance role, or ADC / `ECLUSE_GOOGLE_PROJECT`)
or a static token.
