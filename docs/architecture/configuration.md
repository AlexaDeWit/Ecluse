...
| `MIRROR_QUEUE_URL` | Yes | Queue identifier for mirror jobs: an SQS queue URL, or a Pub/Sub `projects/<project>/topics/<topic>` resource, per provider. Required by the env layer for every provider; **ignored** by `memory` (which has no external queue) — relaxing this requirement under `memory` is tracked in [#318](https://github.com/AlexaDeWit/Ecluse/issues/318). |
