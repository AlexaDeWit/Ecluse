...
| `PROXY_TELEMETRY_ALLOW_PUBLIC_EGRESS` | No (default: `false`) | Telemetry exports **Agent-only** by default; a telemetry endpoint that resolves to a public (off-cluster) address fails the boot loudly unless this is `true` — the explicit opt-in for deliberate remote/agentless export. See [Observability](observability.md#configuration). |
