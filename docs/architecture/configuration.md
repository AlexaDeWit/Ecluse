...
| `MIRROR_TARGET_CODEARTIFACT_DOMAIN_OWNER` | `codeartifact` provider only | The 12-digit account number owning the domain. Resolved from this key, else parsed from the mirror-target host. |
| `MIRROR_TARGET_CODEARTIFACT_REGION` | `codeartifact` provider only | The region of the CodeArtifact domain. Resolution order: this key → `AWS_REGION` → the mirror-target host. |
