...
| **Dependencies** | **Deliberately not modelled** (nor parsed off the wire). | A dependency only ever matters when it is itself fetched, and that fetch comes back through this same gate and receives its own verdict -- gating a parent's dependency *list* would duplicate the gate on every child request. The raw document still relays the lists to clients untouched; a heavy packument carries thousands of per-version dependency entries of pure parse cost, so the decision surface skips them (a malformed entry there degrades rather than dropping the version). Restore the `Dependency`/`DepKind` vocabulary from history if a dependency-reading rule is ever designed. |
...
6. **Cross-upstream merge**, a served packument is the merge of several upstreams' `PackageInfo`. See [Registry Model → Packument merge](registry-model.md#packument-merge-across-upstreams) for details on how provenances (trusted vs gated) are merged.
