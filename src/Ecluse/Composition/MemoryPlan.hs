...
heap ceiling between __named tenants__ whose sum stays within the ceiling. The
...
4. __The material aggregate__: response materialisation working space. The
   ...
   actually hold. That bound is one envelope per admitted operation
   ('packumentOriginFanout' concurrent origins, each wire+parsed at
   'expandWireBytes').
...
6. __The queue tenant__: the in-memory mirror queue's depth, charged only when the
   selection chose the memory backend. That selection precedes this plan, and an SQS
...
7. __The mirror-artifact tenant__: the transient publish envelope one back-fill
   ...
   strict publish document. The tarball bytes, the base64 'Text', and the serialised
   document therefore coexist before GC at roughly
   'mirrorArtifactEnvelopeMultiplier' times the tarball. The worker's per-artifact
   byte cap ('matMaxBytes') is this share divided back down by that multiplier. The
   envelope the cap admits is exactly what the tenant charges.

The combined invariant holds by construction for computed shares: reserve, cache,
material, publish, queue, fixed buffers, and mirror-artifact all fit within the
ceiling. Attribution re-checks explicit overrides, as described under degradation
below. A pin refuses the boot only when it is what a fitting plan cannot shed around,
never on mere presence.

== Graceful degradation, never a refusal

A pod too small for the tenants' normal floors sheds in a documented priority order.
Each step is a loud boot warning naming what was given up and why:

1. The mirror-artifact cap shrinks, to zero if needed. The background back-fill leg
   gives way before the serve hot path. A starved pod therefore protects its cache
   and its admission, and mirrors nothing it cannot buffer safely.
2. The cache aggregate shrinks below its floor, to zero if needed. The proxy then
   serves uncached.
3. Admission shrinks toward one in-flight operation. Where the nursery is the real
   pressure, the capability count sheds with it, which automates the smaller-cores
   recommendation.
4. The publish aggregate shrinks to one maximum request.
5. The queue depth shrinks to its floor.

One operation on one capability with no cache is the irreducible minimum, and it
__always boots__. If even that exceeds the ceiling, the plan says so in its loudest
warning and boots anyway, leaving the cgroup backstop as the guard.

Only an __explicit operator override__ refuses the boot, and only when it is the
cause. The plan re-derives the override-free minimum, with every pin substituted out
and every computed tenant at its floor. It refuses just when that minimum fits the
ceiling while the pinned plan does not. It then names the pins whose individual
removal would fit, or all of them when only their combination overshoots. A pin at or
below the value the shed ladder would compute anyway pushes the plan past nothing and
never refuses. A pod too small even without the pins boots with the loud warning like
any other. An override is an operator claim, and a claim that alone breaks the
ceiling is a misconfiguration to fix, not to shrink around.
...
-- A pure function of the outcomes and the nursery and irreducible-overshoot context.
