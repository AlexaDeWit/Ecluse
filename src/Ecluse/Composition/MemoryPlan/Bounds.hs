...
{- | The shipped numbers the memory plan resolves against, and the byte charge a tenant's
demand turns into. The shares carve up the application heap, the floors and caps bracket
every computed bound, and the fallbacks stand in for a pod with no heap-ceiling datapoint.
A configured value overrides a bound. These are what is left when none is configured.
-}
