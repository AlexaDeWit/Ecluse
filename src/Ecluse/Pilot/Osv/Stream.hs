...
("Ecluse.Runtime") are what bound a small-but-deep payload.
-}
...
import Ecluse.Pilot.Osv (ExtractedOsv, OsvAdvisory, extractFromAdvisory, osvId)
...
streamOsvUrl :: (MonadResource m, MonadThrow m, KatipContext m) => Telemetry -> OsvIngest -> String -> ConduitT i ExtractedOsv m ()
streamOsvUrl telemetry ingest urlStr = do
    ...
    bracketP
        ...
        ( \_ -> do
            ...
            httpSource req (\res -> getResponseBody res .| parseOsvStream telemetry ingest)
        )

-- | Parse the zip stream and emit ExtractedOsv, bounded by @ingest@.
parseOsvStream :: (MonadResource m, MonadThrow m, KatipContext m) => Telemetry -> OsvIngest -> ConduitT ByteString ExtractedOsv m ()
parseOsvStream telemetry ingest = do
