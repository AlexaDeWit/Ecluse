...
- [x] **Filter `versions`**: every version evaluated; denied (and, later,
  undecidable, S21) versions removed from `versions` **and** `time`.   _rules-engine.md#applying-verdicts-to-a-packument_
- [x] **Repoint `dist-tags`**: `latest` resolved by `Ecluse.Version.selectLatest`
  ...
  other tags pointing at a removed version are **dropped**, not repointed.   _rules-engine.md#applying-verdicts-to-a-packument_
- [x] **No survivors** → return `NoSurvivors [Decision]` (the `FilterResult` sum) for
  ...
  cause once S21 lands). This slice does not choose the status.   _rules-engine.md#applying-verdicts-to-a-packument_
...
**Scope boundary.** This slice filters/rewrites a **single (public) packument**, the gated set. Combining that filtered set with the **trusted private** set is the
