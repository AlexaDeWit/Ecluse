...
module Ecluse.Core.Security.HostSmokeSpec (spec) where
...
{- | Smoke tier: a /generative, live/ differential between the SSRF literal recogniser in
"Ecluse.Core.Security" and the __real__ libc resolver. The recogniser reads a leading-zero octet as
octal and a @0x@ octet as hex, exactly as @inet_aton@ and so 'Network.Socket.getAddrInfo' coerce
them, so the resolved address is the ground-truth oracle and this module hard-codes no verdict.

The recogniser deliberately does /not/ model the short @inet_aton@ forms (a bare 32-bit number, a
@127.1@). It treats them as names the host allowlist constrains. Resolution is numeric
(@AI_NUMERICHOST@) and needs no network. It __pends__ if 'getAddrInfo' cannot resolve a plain
literal.
-}
spec :: Spec
...
{- | A four-part dotted-quad spelling whose octets mix decimal, leading-zero octal, and @0x@ hex,
spanning the @inet_aton@ coercions the recogniser models. One spelling in ten corrupts an octet
with a malformed token both the recogniser and the resolver reject.
-}
...
{- | Four octet /values/ for an address, biased toward the internal ranges so the blocked arm is
exercised. Uniform random bytes almost never land in RFC1918.
-}
genOctets :: Gen [Word8]
genOctets =
    Gen.frequency
...
{- | Resolve @host@ as a numeric IPv4 literal through the real 'getAddrInfo', returning 'Nothing'
when the resolver rejects it. The @AI_NUMERICHOST@ flag forbids a DNS lookup, so a non-numeric
spelling fails locally rather than touching the network.
-}
