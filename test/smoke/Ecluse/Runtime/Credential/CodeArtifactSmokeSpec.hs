...
module Ecluse.Runtime.Credential.CodeArtifactSmokeSpec (spec) where
...
> ECLTEST_SMOKE_CODEARTIFACT_REGION=us-east-1 \
> ECLTEST_SMOKE_CODEARTIFACT_DOMAIN=my-sandbox-domain \
> cabal test ecluse-smoke

The optional @ECLTEST_SMOKE_CODEARTIFACT_DOMAIN_OWNER@ overrides the owning account when the
...
spec = describe "live CodeArtifact GetAuthorizationToken" $
    it "mints a non-empty bearer token with an expiry" $ do
        mRegion <- lookupEnv "ECLTEST_SMOKE_CODEARTIFACT_REGION"
        mDomain <- lookupEnv "ECLTEST_SMOKE_CODEARTIFACT_DOMAIN"
        mOwner <- lookupEnv "ECLTEST_SMOKE_CODEARTIFACT_DOMAIN_OWNER"
        case (mRegion, mDomain) of
            ...
            _ ->
                pendingWith
                    ...
                    \(set ECLTEST_SMOKE_CODEARTIFACT_REGION + _DOMAIN); smoke test skipped"
