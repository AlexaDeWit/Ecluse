    ...
    describe "planCveSync -- the per-ecosystem advisory-sync plan" $ do
        ...
        it "plans one handle per configured mount ecosystem and prepares the data dir" $
            withSystemTempDirectory "ecluse-proxy-plan" $ \dir -> do
                ...
                writeFileBS (dataDir </> "npm-osv-schema1.db.tmp") "stale partial download"
                writeFileBS (dataDir </> "npm-osv-schema1.db") "prior artifact"
                cfg <-
                    appConfigFrom
                ...
                for_ (Map.lookup Npm plan) $ \handle -> do
                    ...
                    syncDbPath (csEnv handle) `shouldBe` dataDir </> "npm-osv-schema1.db"
                ...
                doesFileExist (dataDir </> "npm-osv-schema1.db.tmp") `shouldReturn` False
                doesFileExist (dataDir </> "npm-osv-schema1.db") `shouldReturn` True
