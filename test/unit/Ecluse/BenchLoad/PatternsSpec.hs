...
spec = describe "finite request patterns" $ do
    let names = ["a", "b", "c", "d", "e", "f", "g", "h"]
        ...
        requestTrace family settings = fromRight (RequestTrace [] []) (makeTrace family settings names)
    it "visits each cold-install name once with one client regardless of fleet size" $ do
        let result = requestTrace ColdInstall knobs
    ...
    it "replays the same lockfile with exact inter-client skew" $ do
        let clients = rtClients (requestTrace CiFleet knobs{pkSkewMicros = 137})
    ...
    it "reserves disjoint private names around the requested common fraction" $ do
        let result = requestTrace Heterogeneous knobs{pkOverlap = 0.5}
    ...
    it "keeps scan finite and visits the entire space every round" $ do
        let result = requestTrace Scan knobs
    ...
    it "schedules restart arrivals independently of client service times" $
        map ctStartMicros (rtClients (requestTrace Restart knobs{pkArrivalMicros = 500_000})) `shouldBe` [0, 500_000]
    it "reproduces Zipf draws with a seed and changes draws with another seed" $ do
        requestTrace Zipf knobs `shouldNotBe` requestTrace Zipf knobs{pkSeed = 1234}
        map (length . ctNames) (rtClients (requestTrace Zipf knobs)) `shouldBe` [12, 12]
    it "counts distinct bytes once despite repeated traffic and keeps the fat tail" $ do
        let result = requestTrace Scan knobs{pkNames = 8}
